...
## Features ##
 ...
 - Supports interactive patterns `NN/KN/NK/KK/NX/KX/XN/IN/XK/IK/XX/IX` with `25519`, `AESGCM|ChaChaPoly`, and `SHA256|SHA512|BLAKE2b|BLAKE2s`.
...
8. Supported interactive patterns are `NN`、`KN`、`NK`、`KK`、`NX`、`KX`、`XN`、`IN`、`XK`、`IK`、`XX`、`IX`. One-way patterns `N`、`K`、`X` are not supported. Key files are mode-dependent:
* initiator local static: `client_private_key_file`
* initiator remote static: `server_public_key_file`
* responder local static: `server_private_key_file`
* responder remote static: `client_public_key_file`
...
Specifies the responder-side Noise protocol. Supported values are `Noise_<PATTERN>_25519_<CIPHER>_<HASH>`, where `<PATTERN>` is one of `NN/KN/NK/KK/NX/KX/XN/IN/XK/IK/XX/IX`.
...
Specifies the initiator-side Noise protocol. Supported values are `Noise_<PATTERN>_25519_<CIPHER>_<HASH>`, where `<PATTERN>` is one of `NN/KN/NK/KK/NX/KX/XN/IN/XK/IK/XX/IX`.
