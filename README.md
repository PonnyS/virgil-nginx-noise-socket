...
## Features ##
 ...
 - Supports `Noise_NK_25519_{AESGCM|ChaChaPoly}_{SHA256|SHA512|BLAKE2b|BLAKE2s}`.
...
8. For `Noise_NK_25519_*_*`, the responder (server) needs a private key and the initiator (client/proxy) needs the responder public key. Keys are generated by means of the test utility of `echo-keygen` from library`Noise-C` (`noise-c/examples/echo/echo-keygen`). Use of the utility is described in [noise-c example echo](http://rweather.github.io/noise-c/example_echo.html). Example private key files are `virgil-nginx-noise-socket/example/server_key_25519` and `virgil-nginx-noise-socket/example/client_key_25519`.
...
Specifies a file with the public key for the proxied noise responder. In `Noise_NK_25519_*_*` this directive is required when `proxy_noise on`.
...
Specifies a file with the secret key in the format of the simple sequence of bytes for the given noise initiator(client). This directive is optional for the currently supported `Noise_NK_25519_*_*` protocols.
...
Specifies the responder-side Noise protocol. Supported values are `Noise_NK_25519_{AESGCM|ChaChaPoly}_{SHA256|SHA512|BLAKE2b|BLAKE2s}`.

```nginx
Syntax: 	noise_prologue value;
Default: 	noise_prologue NoiseSocketInit1;
Context: 	noise_socket, server
```

Specifies the responder-side Noise prologue text.
...
Specifies the initiator-side Noise protocol. Supported values are `Noise_NK_25519_{AESGCM|ChaChaPoly}_{SHA256|SHA512|BLAKE2b|BLAKE2s}`.

```nginx
Syntax: 	proxy_noise_prologue value;
Default: 	proxy_noise_prologue NoiseSocketInit1;
Context: 	noise_socket, server
```

Specifies the initiator-side Noise prologue text.
